...
public class PostServiceTest {
    ...
    public void setUp() {
        ...
        this.parent = new User("email@test.com", "user", "img");
        this.user = new User("teste@email.com", "teste", "img");
        this.post = new Post(this.parent, "Texto de teste.", img, PostTypes.NEWS, "Titulo Teste");
        this.userService.save(this.user);
        this.userService.save(this.parent);
    }
    ...
    public void testSaveEmailEmpty() {
        Post post = new Post(new User(), "Texto de teste.", "",  PostTypes.ENTERTAINMENT, "Titulo Teste");
        try {
    ...
    public void testSaveEmailNull() {
        Post post = new Post(null, "Texto de teste.", "",  PostTypes.ENTERTAINMENT, "Titulo Teste");
        try {
            this.postService.save(post);
            Assert.fail("Uma exceção deveria ter sido lançada.");
        } catch (Exception e) {
    ...
    public void testSavePostTypeNull() {
        Post post = new Post(new User("email@test.com", "user", "image"), "Texto de teste.", "",  null, "Titulo Teste");
        try {
            this.postService.save(post);
            Assert.fail("Uma exceção deveria ter sido lançada.");
        } catch (Exception e) {
    ...
    @Test
    public void testSavePostNull() {
        try {
            this.postService.save(null);
            Assert.fail("Uma exceção deveria ter sido lançada.");
        } catch (Exception e) {
            Assert.assertFalse("O post não deveria ter sido salvo.", this.postService.getAll().contains(post));
        }
        ...
        try {
          ...
          Assert.fail("Uma exceção deveria ter sido lançada.");
            ...
            Assert.assertArrayEquals("As listas deveriam ser iguais.",
                    posts.toArray(), this.postService.getAll().toArray());
        }
    }

    /**
     * Testa a adição de comentário em um post.
     */
    @Test
    ...
    /**
     * Testa a adição de um comentário null em um post.
     */
    @Test
    public void testAddCommentNull() {
        ...
        try {
            ...
            Assert.assertFalse("O comentário não deveria ter sido inserido.", post.getComments().contains(comment));
        }
    }
     ...
     */
    @Test
    public void testGetCommentsPostNonexistent() {
        Long id = Long.valueOf(-1);
        try {
            this.postService.getComments(id);
            Assert.fail("Uma exceção deveria ter sido lançada.");
        } catch (Exception e) {
           Assert.assertEquals("As mensagens deveriam ser iguais.", e.getMessage(), "No value present");
        }
    }

    /**
     * Testa se um post é alterado corretamente.
     */
    @Test
    public void testUpdate() {
        Post postSaved = this.postService.save(this.post);
        String mensagem = "Alteração de texto.";
        postSaved.setText(mensagem);
        this.postService.update(postSaved.getId(), postSaved);
        Post post = this.postService.getAll().stream().filter(p -> p.getId().equals(postSaved.getId())).findFirst().get();
        Assert.assertEquals("O texto deveria ter sido alterado.", mensagem, post.getText());
    }
}
