...
public class SpottedControllerTest {
    ...
    public void testSave() {
        ResponseEntity<Spotted> spotted = template.postForEntity("/api/spotted", this.spotted, Spotted.class);
        Assertions.assertThat(spotted.getStatusCodeValue()).isEqualTo(200);
    }
    ...
    public void testDelete() {
        ...
        Assertions.assertThat(spotted.getStatusCodeValue()).isEqualTo(200);
        try {
            template.delete("/api/spotted/{id}", spotted.getBody().getId(), Spotted.class);
            ResponseEntity<List> spotteds = template.getForEntity("/api/spotted", List.class);
            Assertions.assertThat(spotteds.getBody()).doesNotContain(spotted);
        } catch (Exception e) {
            Assert.fail("Não deveria ter dado falha.");
        }
    }
    ...
    public void testSetVisible() {
        ...
        for (int i = 0; i < 5; i++) {
            template.put("/api/spotted/{id}", spotted.getBody().getId(), Spotted.class);
        }

        ResponseEntity<List> spottedsAfter = template.getForEntity("/api/spotted/visible", List.class);
        Assertions.assertThat(spottedsAfter.getBody()).doesNotContain(spotted);
    }
}
